...
def check_directory():
    ...
    if "src" in files:
        ...
        if "path_finding.py" in src_files:
            return True
...
def test_part1(grid, GVD, start=(83, 16), goal=(7, 8)):
    from src.path_finding import GVD_path
    from src.utils import PathPlanMode
...
def test_part2(grid, GVD, start=(50, 0), goal=(7, 8)):
    from src.path_finding import cell_to_GVD_gradient_ascent
...
def test_part3(grid, GVD, start=(95, 80), goal=(25, 8)):
    from src.path_finding import cell_to_GVD_a_star
